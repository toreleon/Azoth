...
export async function runBacktestSession(
  ...
  const vnindexAt = (asOf: number): number | null => {
    // ⚡ Bolt: Use O(log n) binary search to find the most recent closing price before asOf
  ...
  try {
    for (const asOf of intervalTurns) {
      ...
      const priceOverride = (sym: string): number | null => {
        ...
        // ⚡ Bolt: Use O(log n) binary search instead of O(n) filtering to find the most recent bar
