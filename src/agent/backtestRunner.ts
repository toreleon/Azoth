...
export async function runBacktestSession(
  ...
  const vnindexAt = (asOf: number): number | null => {
    const index = findLastBarIndex(vnindex, asOf);
    return index !== -1 ? vnindex[index]!.close : null;
  };
  ...
  try {
    for (const asOf of intervalTurns) {
      ...
      const priceOverride = (sym: string): number | null => {
        ...
        const index = findLastBarIndex(series, asOf);
        return index !== -1 ? series[index]!.close : null;
      };
