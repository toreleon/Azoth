...
// ⚡ Bolt Optimization: Use read-only objects and direct indexing to avoid .slice() allocation
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // ⚡ Bolt Optimization: Avoid .map() and .slice().reduce() by directly iterating over bars
