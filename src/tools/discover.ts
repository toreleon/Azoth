...
// ⚡ Bolt Optimization: Avoid O(n) array allocations by operating directly on the source array
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // ⚡ Bolt Optimization: Replace O(n) .map() and .slice().reduce() with direct array indexing
  // and in-place loops to avoid unnecessary object allocations and reduce GC overhead.
  ...
  if (len >= 25) {
    ...
    for (let i = Math.max(0, len - 25); i < priorEnd; i++) {
