...
// Direct indexing replaces .map(), .slice(), and .reduce() calls.
// This significantly reduces GC pressure on the hot path for screening large ticker universes.
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // ⚡ Bolt Optimization: Avoid `.slice().reduce()` allocating arrays inside a map callback
  let recentVolSum = 0;
