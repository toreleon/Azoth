...
// Modified rsi14 to accept source objects and use manual loops
// to avoid O(N) array allocation overhead from map() and slice().
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // Bolt Performance Optimization:
  // Use direct indexing from the end of the array instead of O(N) map() and slice()
