...
// ⚡ Bolt Optimization: Use direct indexing and in-place math to avoid O(N) array slice allocations
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // ⚡ Bolt Optimization: Replace chained .map(), .slice(), and .reduce() with direct indexing
  // and explicit loops to eliminate intermediate array allocations in a hot path.
  const len = bars.length;
