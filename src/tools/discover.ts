...
// ⚡ Bolt Optimization: Replace closes array with readonly bars array to avoid O(N) .map() allocation
// ⚡ Bolt Optimization: Use indexed loop with Math.max to avoid .slice() array allocation overhead
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // ⚡ Bolt Optimization: Avoid O(N) allocations for `closes` and `vols` arrays
  const last = bars[len - 1]!.close;
  const prev1w = bars[len - 6]!.close;
  const prev1m = bars[len - 22]!.close;

  // ⚡ Bolt Optimization: Use in-place loops to avoid .slice() and .reduce() allocations
  let recentVolSum = 0;
