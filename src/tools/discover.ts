...
// Optimized RSI calculation to avoid array allocations from .slice() and .map()
function rsi14(bars: readonly { close: number }[]): number | null {
  const len = bars.length;
  if (len < 15) return null;
  let gains = 0;
  let losses = 0;
  const start = Math.max(1, len - 14);
...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // Direct indexing instead of mapping to avoid intermediate array allocations
  const last = bars[len - 1]!.close;
  const prev1w = bars[len - 6]?.close;
  const prev1m = bars[len - 22]?.close;

  // Calculate recent volume without .slice() and .reduce()
  let recentVolSum = 0;
  ...
  // Calculate prior volume without .slice() and .reduce()
  let priorVol = null;
  if (len >= 25) {
    let priorVolSum = 0;
