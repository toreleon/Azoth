...
async function buildCandidate(ticker: string): Promise<Candidate> {
  ...
  // OPTIMIZATION: Avoid chaining .map(), .slice(), and .reduce() to reduce memory allocation
  // and GC overhead in hot paths. Calculate metrics in-place directly from the source array.
  const len = bars.length;
  const last = bars[len - 1]?.close ?? 0;
