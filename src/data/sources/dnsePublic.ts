...
/**
 * O(log N) binary search for the index of the last bar at or before `time`.
 * Expects chronologically sorted arrays.
 * Returns -1 if no such element exists.
 */
export function findLastBarIndex(bars: { time: number }[], time: number): number {
  let low = 0;
