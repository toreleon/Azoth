...
/**
 * Binary search to find the index of the last bar in a chronologically sorted array
 * that is less than or equal to a target timestamp.
...
function clipBars(bars: Bar[]): Bar[] {
  ...
  // ⚡ Bolt: Use O(log n) binary search instead of O(n) filter to clip chronologically sorted bars
