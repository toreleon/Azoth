...
## 2025-02-12 - Prevented O(N) array allocation overhead in discover.ts
**Learning:** In a highly concurrent scanning map loop like `buildCandidate` running for thousands of equities, chained `.map().slice().reduce()` incurs heavy allocation overhead and forces GC pauses. Refactoring array methods to compute derived indicators (like RSI and volume averages) directly via indexed access on a shared `bars` array provided significant latency improvements without degrading readability.
**Action:** Always prefer computing technical indicators in-place over pre-allocating new array slices in hot paths.
