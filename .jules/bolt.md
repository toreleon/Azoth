...
## 2024-05-18 - Avoid map/reduce/slice chains in array loop evaluations
**Learning:** `src/tools/discover.ts` was doing multiple array iterations (`.map()`, `.slice().reduce()`) internally inside a tight loop (`discoverTickers`), which causes a large number of redundant garbage collection steps and intermediate array memory allocations.
**Action:** When computing aggregates inside frequent evaluation paths for large inputs (like technical indicators or market metrics), write in-place bounds-checked loops against the initial source array rather than relying on immutable method chaining.
