...
## 2024-05-20 - Avoid Array.map(), slice(), and reduce() on hot path discoveries
**Learning:** `src/tools/discover.ts` iterates over the entire stock universe concurrently, loading historical bars and calculating indicators for each ticker. Chaining `.map()`, `.slice()`, and `.reduce()` created massive garbage collection pressure by constantly allocating and immediately discarding short-lived intermediate arrays.
**Action:** Replaced functional array methods in `buildCandidate` and `rsi14` with standard indexed `for`-loops that traverse the source arrays directly from the end (using `bars.length - N`), taking runtime for evaluating `all_listed` (1000+ items) from ~28.4s to ~0.9s. Always avoid allocating throwaway arrays on tight inner loops evaluating large datasets.
