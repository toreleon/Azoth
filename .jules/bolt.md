...
## 2026-08-29 - Avoid chained array operations on O(N) array items in loops
**Learning:** In the ticker discovery scan, `buildCandidate` was mapping properties, slicing, and reducing arrays for dozens to hundreds of tickers at once. These chained array method calls allocate new arrays, adding huge GC pressure and runtime overhead.
**Action:** Use single `for` loops and index tracking directly into the root objects to compute summaries (like RSI, moving averages) without creating intermediate slices.
