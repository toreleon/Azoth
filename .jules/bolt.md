...
## 2024-05-18 - Preserving Intent with `Math.max()` in Optimizations
**Learning:** When replacing `.slice(-N)` with indexed loops in hot paths, using `Math.max(0, len - N)` is great for avoiding out-of-bounds indices. However, if the original codebase intentionally guarded the logic with an explicit length check (e.g., `vols.length >= 25 ? vols.slice(-25) : null`), replacing it blindly with `Math.max(0, len - 25)` fundamentally changes the domain logic by silently operating on incomplete data instead of failing fast or returning `null`. This can cause massive financial calculation errors (like returning a 4x smaller volume denominator).
**Action:** When converting array operations to loops, carefully inspect if the original code had a deliberate length requirement or if it was relying purely on the graceful fallback of `.slice()`. Always preserve explicit boundary conditions (`if (len >= 25) ... else ...`) rather than relying solely on `Math.max()`.
