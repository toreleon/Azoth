...
## 2026-08-27 - [Optimize Ticker Discovery Memory Allocations]
**Learning:** Chained array methods (.map, .slice, .reduce) in hot paths (like ticker discovery over many items) generate unnecessary GC overhead and intermediate array allocations. Directly iterating over source arrays using bounds logic avoids this cost.
**Action:** When filtering or mapping data during frequent loops on arrays, refactor using in-place indexed for-loops or pass arrays directly instead of mapping primitives, particularly in tools like `discover.ts`.
