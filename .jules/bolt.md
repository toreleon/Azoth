...
## 2024-05-18 - Avoid array allocations in hot paths like discoverTickers
**Learning:**  created many garbage arrays with `.map(b => b.close)`, `.slice()`, and `.reduce()` inside `buildCandidate` / `rsi14` which is run for every ticker. This O(N) allocation per ticker creates GC pauses and overhead.
**Action:** Use direct array indexing (`bars[len - 1].close`) and write standard `for` loops from the end of the array rather than mapping or slicing out subsets of the array.

## 2024-05-18 - Avoid array allocations in hot paths like discoverTickers
**Learning:** `discoverTickers` created many garbage arrays with `.map(b => b.close)`, `.slice()`, and `.reduce()` inside `buildCandidate` / `rsi14` which is run for every ticker. This O(N) allocation per ticker creates GC pauses and overhead.
**Action:** Use direct array indexing (`bars[len - 1].close`) and write standard `for` loops from the end of the array rather than mapping or slicing out subsets of the array.
