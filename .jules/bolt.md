...
## 2024-05-18 - Avoid unnecessary array allocations in hot paths like discover
**Learning:** Functions evaluating indicators over historical data frequently rely on chaining array methods (`.map`, `.slice`, `.reduce`). When applied repetitively over large datasets, such as during discovery tasks running concurrently for thousands of tickers, these methods introduce high garbage collection overhead and slow down the process significantly. In particular, `buildCandidate` in `src/tools/discover.ts` had a measurable performance bottleneck.
**Action:** Replace functional array method chains with simple `for` loops utilizing indices to directly read from the original array. This avoids creating intermediary arrays and saves significant execution time and GC pauses. Use the optional chaining operator `?.` to gracefully handle lengths when retrieving prices.
