...
## 2024-05-19 - Avoid Chaining Array Methods in Hot Paths
**Learning:** In `src/tools/discover.ts`, calculating technical indicators (like RSI and volume ratios) across thousands of tickers used chained array methods (`.map().slice().reduce()`). This caused unnecessary memory allocation and garbage collection overhead.
**Action:** When calculating metrics over arrays in hot paths (like ticker discovery), replace chained array methods with in-place indexed `for` loops. This avoids allocating intermediate arrays and improves throughput significantly (e.g., ops/sec increased from 0.2 to 2.8 in discovery benchmarking).
