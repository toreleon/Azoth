...
## 2025-02-12 - Prevent Array Allocations in Ticker Discovery Hot Path
**Learning:** In highly repetitive data aggregations over market ticks/bars, chaining `.map()` to extract primitive arrays (like `closes` or `vols`) and then chaining `.slice(-N).reduce(...)` generates significant garbage collection overhead and intermediate object allocation.
**Action:** When extracting sub-intervals from chronologically sorted time-series object arrays, access properties directly via indexing into the source array (`bars[i]!.volume`). Replace `.slice(-N).reduce(...)` aggregations with indexed `for` loops iterating from the end (`Math.max(0, len - N)`). Use optional chaining (`?.`) when directly plucking end elements to handle arrays safely when length may vary.
