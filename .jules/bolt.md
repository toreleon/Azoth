...
## 2024-08-12 - [Array map allocations in hot paths]
**Learning:** In high-throughput, concurrent code paths like `discoverTickers` (which iterates over hundreds of tickers via `Promise.all` mapping), chaining array methods like `.map().slice().reduce()` creates significant intermediate garbage collection overhead.
**Action:** When implementing ticker metric calculations across large sets, use indexed `for` loops directly on the original `bars` array instead of extracting column-wise arrays, specifically avoiding `.map(b => b.close)` and `.map(b => b.volume)`.
