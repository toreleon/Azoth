...
## 2024-11-20 - Array operations in discover.ts hot path
**Learning:** `buildCandidate` and `rsi14` in `src/tools/discover.ts` are called up to ~1500 times per ticker discovery run. The use of `.map()` to create intermediate `closes` and `vols` arrays, combined with `.slice()` and `.reduce()`, led to significant garbage collection overhead and execution time (800ms+). By replacing these with in-place loop accumulations (`Math.max(0, len - N)` for bounds) and directly accessing the source `bars` array, the execution time was reduced to ~300ms.
**Action:** In hot loops, especially those processing large market data arrays across multiple tickers, avoid chaining `.map()`, `.slice()`, and `.reduce()`. Use manual `for` loops with calculated bounds to eliminate intermediate array allocations.
