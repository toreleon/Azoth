...
## 2024-05-18 - Avoid unnecessary array allocations in ticker discovery hot path
**Learning:** In `buildCandidate` (`src/tools/discover.ts`), mapping over the `bars` array to create separate `closes` and `vols` arrays, followed by `.slice()` and `.reduce()`, introduces massive memory allocation and GC overhead when scanning hundreds of tickers concurrently.
**Action:** Replaced chained array methods with in-place indexing (`bars[len - N]`) and single-pass `for` loops directly on the source array, cutting out the intermediate object creation while preserving existing functionality.
