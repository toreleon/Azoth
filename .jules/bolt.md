...
## 2024-05-19 - Avoid array map and slice in hot ticker discovery paths
**Learning:** In `src/tools/discover.ts`, array methods `.map`, `.slice`, and `.reduce` were used to compute indicator metrics across hundreds of tickers in `buildCandidate()`. This caused excessive garbage collection overhead.
**Action:** Replace `.map` with direct index access on the source `bars` array and use `for` loops rather than `.slice().reduce()` to prevent temporary array allocations. Avoid unnecessary `.map` calls when possible.
