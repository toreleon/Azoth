...
## 2024-05-18 - Optimize Ticker Discovery by Removing Array Allocations
**Learning:** In highly repetitive data transformation logic (like scanning hundreds of ticker candidates inside `buildCandidate`), avoiding high-level JS array methods (`.map`, `.slice`, `.reduce`) and replacing them with raw indexed `for` loops directly over the original fetched objects greatly reduces garbage collection and memory allocations.
**Action:** When refactoring time-series calculation code in hot paths, directly accept the original array of objects and use index arithmetic (e.g., `len - N`) along with manual summation loops, instead of allocating intermediate arrays via `.map` or `.slice`.
