...
## 2024-05-18 - Optimize array allocation in discover.ts
**Learning:** The `discoverTickers` function iterates over many tickers, computing indicators and doing array manipulations like `.map()` and `.slice().reduce()` on each OHLCV series. This creates large numbers of short-lived objects for every ticker candidate, resulting in GC overhead and unnecessary allocations.
**Action:** Avoid primitive array extraction and chaining of array methods in hot paths (like candidate discovery). Iterate over source data arrays directly with indexed `for` loops and compute values in-place.
