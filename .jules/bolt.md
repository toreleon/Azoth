...
## 2025-02-18 - [Zero-Allocation Aggregation in Ticker Discovery]
**Learning:** Chained array methods like `.map()`, `.slice()`, and `.reduce()` inside tight async map limits (like `discoverTickers` testing hundreds of symbols) cause significant GC pressure and slowdowns.
**Action:** When computing multi-interval metrics (e.g. RSI, moving averages) from arrays of objects (like OHLCV data), avoid mapping to intermediate arrays. Instead, pass the original object array as `readonly`, calculate starting indices using `Math.max(0, len - N)`, and use standard `for` loops directly over the original objects' properties.
