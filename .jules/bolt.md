...
## 2024-05-18 - Ticker Discovery Bulk Slicing GC Overhead
**Learning:** Functions like `buildCandidate` in `src/tools/discover.ts` are called for hundreds of tickers. Chaining `.map()` and `.slice(-N)` allocations on large OHLCV bars arrays inside this loop creates substantial garbage collection overhead, converting an O(n) traversal into an O(n^2) allocation problem where n is the universe size.
**Action:** Replace map/slice chains inside bulk discovery loops with direct `bars[i]` indexing and standard `for` loops to eliminate intermediate array allocations.
