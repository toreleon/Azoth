...
## 2024-05-18 - [Optimize candidate selection by removing array allocations]
**Learning:** In hot loops over large ticker universes, doing chained array manipulations (like `.map()`, `.slice()`, and `.reduce()`) inside each processing step creates massive garbage collection pressure, severely slowing down execution time.
**Action:** Always replace chained array operations with manual, in-place `for` loops and direct indexing from the end of the array (using `len - N` and optional chaining `?.`) for time-series computations (like RSI and volume ratios) in performance-critical paths.
